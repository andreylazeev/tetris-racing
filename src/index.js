...
const hero = new Car(app, 40, BASE_HEIGHT - 240, false)
let enemies = []
ticker.add((delta) => {
  if (!enemies.length) {
    const enemy = new Car(app, Math.random() > 0.5 ? 40 : BASE_WIDTH - 300, -160, true)
    enemies.push(enemy)
  } 
  ...
  if (enemies[enemies.length - 1].root.y - 200 >= 0) {
    const enemy = new Car(app, Math.random() > 0.5 ? 40 : BASE_WIDTH - 300, -160, true)
    enemies.push(enemy)
    return true
  }
  ...
  enemies.some(enemy => {
    if (checkForCollision(enemy.root, hero.root)) {
      console.log('lose');
      return true
    }
  })
